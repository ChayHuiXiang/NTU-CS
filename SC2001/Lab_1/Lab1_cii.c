...
int main() {
    int n, i, count = 0;
    FILE* fpt;
    fpt = fopen("data_cii.csv", "w");
    fprintf(fpt,"n, S, comparisons\n");

    n = 100000;
    for (int SORT_THRESHOLD = 2; SORT_THRESHOLD <= MAX; SORT_THRESHOLD++) {
        int* arr = (int*)malloc(sizeof(int) * n);
        srand(SORT_THRESHOLD);
        for (i = 0; i < n; i++) {
            arr[i] = rand() % n + 1;
        }
        count = 0;
        hybridSort(arr, 0, n - 1, &count, SORT_THRESHOLD);
        printf("For S = %d, number of key comparisons = %d\n", SORT_THRESHOLD, count);
